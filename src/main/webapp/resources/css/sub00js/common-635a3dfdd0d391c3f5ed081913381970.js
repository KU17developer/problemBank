...
function openMultiMusic(ids) {
    if (!checkUserLoggedIn()) {
        return;
    }

    let url = "/common/star_music_player.html?type=list&ids=" + ids.join(",");
    if (currentDevice == 'MOBILE') {
        window.open(url);
    } else {
        Common.openPop(478, 561, url, "no", "win_multi_music_player");
    }
}
...
function openMyMusic() {
    ...
    let url = "/common/star_music_player.html?type=my";
    if (currentDevice == 'MOBILE') {
        window.open(url);
    } else {
        Common.openPop(478, 561, url, "no", "win_multi_music_player");
