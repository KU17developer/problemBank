...
function PostSubmit(u, p, t) {
    // description : Post 방식으로 페이지를 이동시킨다.
    ...
    for (var k in p) {
        ...
        if (typeof v === "object") {
            nv = '';
            $.each(v, function (e, va) {
                if (e > 0) {
                    nv += ',';
                }
                nv += va;
            });
            objs.setAttribute('value', nv);
            nv = '';
        } else {
...
function getSubmit(u, p, t) {
    // description : GET 방식으로 페이지를 이동시킨다.
